...
const Sound = {
  ...
  move(){ this._play(480,'sine',.08,.18); },
  capture(){ this._play(300,'triangle',.05,.3); setTimeout(()=>this._play(180,'triangle',.12,.25,120),30); },
  king(){ [523,784,1047].forEach((f,i)=>setTimeout(()=>this._play(f,'sine',.18,.2),i*80)); },
  win(){ [523,659,784,1047].forEach((f,i)=>setTimeout(()=>this._play(f,'sine',.25,.22),i*120)); },
  lose(){ [440,370,311,277].forEach((f,i)=>setTimeout(()=>this._play(f,'triangle',.22,.20),i*130)); },
};
...
Object.assign(window, {
    ...
    startCheckersAI, startCheckersWhiteboard,
});
